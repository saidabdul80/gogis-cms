...
const fullName = computed(() => {
    return `${props.customer.first_name} ${props.customer.middle_name || ''} ${props.customer.last_name}`.trim()
})
...
const fetchPaymentHistory = async (propertyId: number) => {
    ...
    try {
        const response = await fetch(route('admin.individual-customers.property-payments', {
            individualCustomer: props.customer.id,
                ...
                <v-col cols="12">
                    <div class="d-flex justify-space-between align-center">
                        <div>
                            ...
                            <p class="text-subtitle-1 text-grey">Individual Customer Profile</p>
                        </div>
                        <div>
                            <Link :href="route('admin.individual-customers.edit', customer.id)">
                                <v-btn color="primary" prepend-icon="mdi-pencil">
                                    Edit Customer
                                </v-btn>
                            </Link>
                            <Link :href="route('admin.individual-customers.index')" class="ml-2">
                    ...
                    <v-card>
                        ...
                        <v-card-text>
                            <v-list density="compact">
                                ...
                                <v-list-item v-if="customer.nin">
                                    <template #prepend>
                                        <v-icon>mdi-card-account-details</v-icon>
                                    </template>
                                    <v-list-item-title>NIN</v-list-item-title>
                                    <v-list-item-subtitle>{{ customer.nin }}</v-list-item-subtitle>
                                </v-list-item>
                                ...
                                <v-list-item v-if="customer.dob">
                                    <template #prepend>
                                        <v-icon>mdi-cake</v-icon>
                                    </template>
                                    <v-list-item-title>Date of Birth</v-list-item-title>
                                    <v-list-item-subtitle>{{ customer.dob }}</v-list-item-subtitle>
                                </v-list-item>
                                ...
                                <v-list-item v-if="customer.lga">
                                    <template #prepend>
                                        <v-icon>mdi-map-marker</v-icon>
                                    </template>
                                    <v-list-item-title>LGA</v-list-item-title>
                                    <v-list-item-subtitle>{{ customer.lga.name }}</v-list-item-subtitle>
                                </v-list-item>
